class PCLootCrate: ZLandContainerBase
{
	override void RegisterLootTable(array<ref ZLandLootEntry> loot_table) 
	{
		...
		loot_table.Insert(new ZLandLootEntry({"CPB_GPU","CPB_GPCoin","CPB_LEDX","ZLAND_NVME","ZLAND_Powerbank"}, 0.1));
		loot_table.Insert(new ZLandLootWithAttachments({"ZLand"}, 0, {
	...
	{
		EffectSound sound =	SEffectManager.PlaySound( "pcOpen_SoundSet", GetPosition() );
