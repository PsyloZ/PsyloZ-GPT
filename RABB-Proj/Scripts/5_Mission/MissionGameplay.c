...
{		
	protected int m_PlayerPing;
	protected float m_NextPingUpdateIn;
	static const int PING_UPDATE_TIME = 5;
    ...
    override void OnInit()
	{
		...
		if (!m_HudRootWidget )
		{
			m_HudRootWidget	= GetGame().GetWorkspace().CreateWidgets("RA_BaseBuilding/gui/layouts/day_z_hud.layout");
	...
	void UpdateServerPing()
	{
		...
		if(!player || !player.GetIdentity() || !player.GetIdentity().GetPingAct()) {
			return;
		}

		m_PlayerPing = player.GetIdentity().GetPingAvg();

		Param1<int> playerPing = new Param1<int>(m_PlayerPing);
		g_Game.RPCSingleParam(player, 345346454577457, playerPing, true);
	}
	...
	{        
		...
		m_NextPingUpdateIn += timeslice;
		if (m_NextPingUpdateIn > PING_UPDATE_TIME) {
			UpdateServerPing();
			m_NextPingUpdateIn -= PING_UPDATE_TIME;
		}
    }
}